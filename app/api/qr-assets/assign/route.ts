  ...
  try {
    ...
    if (!session?.user?.id) {
      return NextResponse.json(
        { error: "You must be signed in to assign QR assets." },
        { status: 401 }
      )
    }
    ...
    // Allow both UNREGISTERED and ACTIVE statuses
    // UNREGISTERED = new assignment, ACTIVE = reassignment
    if (qrAsset.status !== "UNREGISTERED" && qrAsset.status !== "ACTIVE") {
      return NextResponse.json(
        { error: `QR asset cannot be assigned (status: ${qrAsset.status}). Only UNREGISTERED or ACTIVE QR assets can be assigned.` },
        { status: 409 }
      )
    }
    ...
    const errorMessage =
      error?.message || "Failed to assign QR asset. Please try again."
