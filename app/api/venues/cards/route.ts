...
import type { VenueCard, VenueCardsResponse, VenueDealBadge, VenueOpenStatus } from "@/types/venue"
...
export async function GET(request: Request) {
  try {
    ...
    if (ids.length > 0) {
      ...
      whereClause.latitude = { gte: parsedBounds.south, lte: parsedBounds.north }
      whereClause.longitude = { gte: parsedBounds.west, lte: parsedBounds.east }
    }
    ...
    if (session?.user?.id && formattedVenues.length > 0) {
      const ids = formattedVenues.map((v) => v.id)
      const favorites = await prisma.favoriteVenue.findMany({
        where: { userId: session.user.id, venueId: { in: ids } },
        select: { venueId: true },
      })
      favoritedVenueIds = favorites.map((f) => f.venueId)
    }

    const response: VenueCardsResponse = { venues: formattedVenues, favoritedVenueIds }
    return NextResponse.json(response)
  } catch (error) {
    console.error("Error fetching venue cards:", error)
    return NextResponse.json({ error: "Failed to fetch venue cards" }, { status: 500 })
  }
}
